...
Linux/macOS 以及打包步骤见 [构建说明](docs/building.md)。架构与线程/保存约束见 [架构说明](docs/architecture.md)，引擎选型和同类项目对比见 [技术调研](docs/research.md)。
...
- 本地 Qt 6.10.2 Windows 便携验证 ZIP（r5）实测 41.429 MiB，解压后依赖检查和 `--version` 冒烟通过；最终用户包不含测试基准程序或 `Qt6Test.dll`。该文件不是正式 Release 资产。
...
See [build documentation](docs/building.md) for Linux/macOS and packaging, [architecture](docs/architecture.md) for threading, ownership, and safe-save rules, and [technical research](docs/research.md) for the engine decision and project comparison.
...
- A local Qt 6.10.2 Windows portable validation ZIP (r5) measured 41.429 MiB and passed extracted dependency and `--version` smoke checks. The end-user package contains neither the benchmark executable nor `Qt6Test.dll`; it is not a formal Release asset.
